...
//
  ...
  G4VPhysicalVolume* silicon_phys
    = new G4PVPlacement(G4Transform3D(rm,G4ThreeVector(std::cos(phi)*0.52501*cm,std::sin(phi)*0.52501*cm,0)), sipm_log,"sipm",
                       world_log,false,0); 

//For a second detector:
//This detector is above the first one, the rotation is always about zero so the two detectors are always aligned.
 G4Box* Detector2_box = new G4Box("Detector2",fdetector_x/2.0,fdetector_y/2.0,fdetector_z/2.0); 

  G4LogicalVolume* Detector2_log
    = new G4LogicalVolume(Detector2_box,PlasticSc,"Detector2",0,0,0);
...
G4VPhysicalVolume* Detector2_phys
    = new G4PVPlacement(G4Transform3D(rm,G4ThreeVector(std::cos(phi)*(7)*cm,std::sin(phi)*(7)*cm,0)), Detector2_log,"Detector2",
                       world_log,false,0);
  G4Box* sipm_box2 = new G4Box("sipm2",(fdetector_x/2.0)/40.,(fdetector_y/2.0)*6/50.,(fdetector_z/2.0)*6/50.);

  G4LogicalVolume* sipm_log2
    = new G4LogicalVolume(sipm_box2,silicon,"sipm",0,0,0);

  G4VPhysicalVolume* silicon_phys2
    = new G4PVPlacement(G4Transform3D(rm,G4ThreeVector(std::cos(phi)*(7+0.51251)*cm,std::sin(phi)*(7+0.51251)*cm,0)), sipm_log2,"sipm",
                       world_log,false,0); 
...
// ----------Aluminum------------
  ...
  G4SubtractionSolid* subtraction1 =
    new G4SubtractionSolid("Box-box", box1, box2);

  G4SubtractionSolid* subtraction2 =
    new G4SubtractionSolid("Box-box", box3, box4);
