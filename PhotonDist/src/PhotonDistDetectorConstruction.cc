...
G4Material* Bc408 = new G4Material("PLASTIC_SC_Bc408", density = 1.043*g/cm3, ncomponents=2);
  ...
  Bc408->AddElement(C, fractionmass=0.922582);
	...
	Bc408_mt->AddProperty("RINDEX", PhotonEnergy, RINDEX_Bc408, NUMENTRIES)->SetSpline(true); //el SetSpline es interpolar
	Bc408_mt->AddProperty("ABSLENGTH", PhotonEnergy, ABSORPTION_Bc408, NUMENTRIES)->SetSpline(true); //para hacer tu funcioncita bonita
...
G4double thiccness;
thiccness = 1*cm;
G4double positionboi;
positionboi = thiccness/2.0 + 3.45*cm;
