...
#include "SourceSimDetectorConstruction.hh"
...
SourceSimDetectorConstruction::SourceSimDetectorConstruction()
...
G4VPhysicalVolume* SourceSimDetectorConstruction::Construct()
{
...
// The Detector
//
  G4Box* Detector_box = new G4Box("Detector",fDetector_y/2.0,fDetector_z/2.0, fDetector_x/2.0);
  ...
  G4VPhysicalVolume* Detector_phys
    = new G4PVPlacement(G4Transform3D(rm,G4ThreeVector(std::cos(phi)*0*cm,std::sin(phi)*0*cm,0)), Detector_log,"Detector",
                       world_log,false,0);
//the sipm   
  G4Box* sipm_box = new G4Box("sipm",(fDetector_y/2.0)*6/50.,(fDetector_z/2.0)*6/50.,(fDetector_x/2.0)/40.);

  G4LogicalVolume* sipm_log
    = new G4LogicalVolume(sipm_box,silicon,"sipm",0,0,0);

  G4VPhysicalVolume* silicon_phys
    = new G4PVPlacement(G4Transform3D(rm,G4ThreeVector(0,std::sin(phi)*0.51251*cm,std::cos(phi)*0.51251*cm)), sipm_log,"sipm",
                       world_log,false,0); 
